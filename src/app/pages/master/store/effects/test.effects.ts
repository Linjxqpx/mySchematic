...
export class Test {
    ...
    constructor(
        ...
        private store: Store<simple>
    ) {}

    @Effect()
    simpleDetail$ = _entry$<string>(this.actions$, 
        simple.MemberAuthActions.LOAD_DETAIL)
        .pipe(
            exhaustMap((payload: string) => {
                ...
                const handleSuccess = (result: AspnetJsonResult<ViewModel>) =>
                    of(new simple.MemberAuthActions.loadDetailSuccessAction(result.element));

                // 失敗時將呼叫 loadDetailFailed$ 進行後續行為
                const handleFailed = (result: AspnetJsonResult<ViewModel>) =>
                    of(new simple.MemberAuthActions.loadDetailFailedAction(result.message));
    ...
    simpleSuccess$ = _entry$<ViewModel>(this.actions$, 
        simple.MemberAuthActions.LOAD_DETAIL_SUCCESS).pipe(
        tap(payload => {
    ...
    simpleFailed$ = _entry$<string>(this.actions$, 
        simple.MemberAuthActions.LOAD_DETAIL_FAILED).pipe(
        tap(payload => {
            console.log('failed');
            console.log(payload)
